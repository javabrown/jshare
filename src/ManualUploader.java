...
public class ManualUploader {
  ...
  public static void main(String[] args) throws Exception{
    //ManualUploader.postToJavaBrown()
    ImageFetcher.fetch();
  }
  
  public static void postToJavaBrown() throws Exception {
    int max = 610;
...
class Base64Encoder {
  ...
  private static InputStream compress(File file, float quality)
      throws IOException {
...
class ImageFetcher {
  public static void fetch() {
    for (int para = 0; para < 610; para++) {
      String urlStr = "http://cms.javabrown.com/qfetcher.php?output=base641&verse=1&chapter="
          + para;
      saveOriginal(urlStr, para);
      compress(urlStr, para);
    }
  }
  
  private static void saveOriginal(String urlStr, int index){
    ...
    try {
      ...
      ImageIO.write(image, "jpg", new File("C:\\tmp\\ii\\quran-page-" + index
          + ".jpg"));
  ...
  private static void compress(String urlStr, int index){
    BufferedImage image = null;
    try {
      System.out.print("Receiving - " + urlStr);
      URL url = new URL(urlStr);
  ...
  private static void fetch99() throws InterruptedException {
    ...
    for (int para = 1; para < 50; para++) {
      for (int page = 1; page < 50; page++) {
        ...
        // http://www.equranacademyJavabrown.com/images/stories/paraIndex+"/"+fileIndex
        String urlStr = "http://www.equranacademyJavabrown.com/images/stories/"
            + paraIndex + "/" + fileIndex;
        ...
        try {
          ...
          ImageIO.write(image, "jpg", new File("C:\\tmp\\ii\\quran-page-"
              + newFileIndex + ".jpg"));
          newFileIndex++;
          // ImageIO.write(image, "gif",new File("C:\\out.gif"));
          // ImageIO.write(image, "png",new File("C:\\out.png"));
  ...
  private static InputStream compressAndSave(BufferedImage image, int index)
    ...
    // ImageOutputStream ios = ImageIO.createImageOutputStream(bos);
    File file = new File("C:\\tmp\\ii-compressed\\quran-page-" + index + ".jpg");
    ImageOutputStream ios = ImageIO.createImageOutputStream(file);

    writer.setOutput(ios);
    IIOImage compressedImage = null;
    writer.write(null, new IIOImage(image, null, null), param);
